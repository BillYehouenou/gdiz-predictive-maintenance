...
class ExogenousContextGenerator:
    ...
    def generate_ambient_temperature(self, timestamps: pd.DatetimeIndex, seasons: np.ndarray) -> np.ndarray:
        ...
        noise = np.where(
            seasons == 1,
            np.random.normal(0, 1.0, len(timestamps)),
            np.random.normal(0, 1.8, len(timestamps))
        )
    ...
    def _handle_downtime_phase(self, failure_idx: int) -> int:
        ...
        for idx in range(start_downtime, end_downtime):
            ...
            self.process_temperature[idx] = (
                alpha_refroidissement * self.ambient_array[idx]
                + (1 - alpha_refroidissement) * self.process_temperature[idx - 1]
            )
    ...
    def evolve_states(self) -> pd.DataFrame:
        ...
        for idx in range(1, self.num_samples):
            ...
            if self.power_loss_array[idx] == 1:
                ...
                alpha = self.config.coef_lissage_temp
                self.process_temperature[idx] = (
                    alpha * self.ambient_array[idx] + (1 - alpha) * self.process_temperature[idx - 1]
                )
...
class DataAggregator:
    ...
    def inject_missing_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_copy = df.copy()
        sensor_cols = [
            'tool_wear', 'process_temperature', 'rotational_speed',
            'torque', 'vibration', 'ambient_temperature', 'humidity', 'dust_concentration'
        ]
...
class SyntheticDatasetOrchestrator:
    ...
    def generate(self) -> pd.DataFrame:
        ...
        machine_type_quality = {
            mid: machine_types_cycle[i % 3]
            for i, mid in enumerate(self.config.machine_ids)
        }
        ...
        if len(df_final) != len(df_sensor):
            raise ValueError(
                f"La fusion a modifié le nombre de lignes ! "
                f"(Sensor: {len(df_sensor)}, Final: {len(df_final)})"
            )
